...
contract PanswapLibraryTest is Test {
    ...
    function setUp() public {
        ...
        tokenA = new MintableToken("TokenA", "TKNA");
        tokenB = new MintableToken("TokenB", "TKNB");
        tokenC = new MintableToken("TokenC", "TKNC");
        tokenD = new MintableToken("TokenD", "TKND");
    ...
    function testGetReserves() public {
        ...
        PanswapPair(address(pair)).mint(address(this));

        (uint256 reserve0, uint256 reserve1) = PanswapLibrary.getReserves(
            address(factory),
            address(tokenA),
            address(tokenB)
        );
    ...
    function testQuote() public {
        uint256 amountOut = PanswapLibrary.quote(1 ether, 1 ether, 1 ether);
        assertEq(amountOut, 1 ether);

        amountOut = PanswapLibrary.quote(1 ether, 2 ether, 1 ether);
        assertEq(amountOut, 0.5 ether);

        amountOut = PanswapLibrary.quote(1 ether, 1 ether, 2 ether);
    ...
    function testGetAmountOut() public {
        uint256 amountOut = PanswapLibrary.getAmountOut(
            1000,
            1 ether,
            1.5 ether
        );
    ...
    function testGetAmountsOut() public {
        ...
        uint256[] memory amounts = PanswapLibrary.getAmountsOut(
            address(factory),
            0.1 ether,
            path
        );
    ...
    function testGetAmountsOutInvalidPath() public {
        ...
        PanswapLibrary.getAmountsOut(address(factory), 0.1 ether, path);
    }

    function testGetAmountIn() public {
        uint256 amountIn = PanswapLibrary.getAmountIn(1495, 1 ether, 1.5 ether);
        assertEq(amountIn, 1000);
    }

    function testGetAmountInZeroInputAmount() public {
        vm.expectRevert(encodeError("InsufficientAmount()"));
        PanswapLibrary.getAmountIn(0, 1 ether, 1.5 ether);
    }

    function testGetAmountInZeroInputReserve() public {
        vm.expectRevert(encodeError("InsufficientLiquidity()"));
        PanswapLibrary.getAmountIn(1000, 0, 1.5 ether);
    }

    function testGetAmountInZeroOutputReserve() public {
        vm.expectRevert(encodeError("InsufficientLiquidity()"));
        PanswapLibrary.getAmountIn(1000, 1 ether, 0);
    }
    ...
    function testGetAmountsIn() public {
        ...
        uint256[] memory amounts = PanswapLibrary.getAmountsIn(
            address(factory),
            0.1 ether,
            path
        );
    ...
    function testGetAmountsInInvalidPath() public {
        ...
        vm.expectRevert(encodeError("InvalidPath()"));
        PanswapLibrary.getAmountsIn(address(factory), 0.1 ether, path);
    }
}
